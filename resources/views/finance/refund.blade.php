        ...
        <div class="col-6">
          ...
          <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
            <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
              ...
              <li class="breadcrumb-item active">退费统计</li>
            </ol>
          </nav>
        </div>
      </div>
          ...
          <div class="card card-stats">
            <div class="card-body">
              <div class="row">
                ...
                <div class="col">
                  <h5 class="card-title text-muted mb-0">退费金额</h5>
                  <span class="h2 font-weight-bold mb-0 text-blue">￥ {{ number_format($total_refund_amount, 2) }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="card card-stats">
            <div class="card-body">
              <div class="row">
                ...
                <div class="col">
                  <h5 class="card-title text-muted mb-0">退费次数</h5>
                  <span class="h2 font-weight-bold mb-0 text-blue">{{ number_format($total_refund_num) }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="card card-stats">
            <div class="card-body">
              <div class="row">
                ...
                <div class="col">
                  <h5 class="card-title text-muted mb-0">退费课时</h5>
                  <span class="h2 font-weight-bold mb-0 text-blue">{{ number_format($total_hour_num) }}</span>
                </div>
              </div>
            </div>
          </div>
              ...
              <table class="table align-items-center table-hover text-left">
                <thead class="thead-light">
                  <tr>
                    ...
                    <th style="width:28%;">退费金额</th>
                    <th style="width:28%;">退费次数</th>
                    <th style="width:28%;">退费课时</th>
                  </tr>
                </thead>
                <tbody>
                  @for($i=0;$i<count($ids);$i++)
                    <tr>
                      <td>{{ $ids[$i] }}</td>
                      @if($refund_amounts[$i]!=0)
                        <td>￥ {{ number_format($refund_amounts[$i], 2) }}</td>
                      @else
                        <td></td>
                      @endif
                      @if($refund_nums[$i]!=0)
                        <td>{{ number_format($refund_nums[$i]) }}</td>
                      @else
                        <td></td>
                      @endif
                      @if($hour_nums[$i]!=0)
                        <td>{{ number_format($hour_nums[$i]) }}</td>
                      @else
                        <td></td>
                      @endif
                    </tr>
...
<script>
  ...
  linkActive('financeRefund');

  function form_submit(){
      document.getElementById('form1').submit();
  }

  barChart("barChart1", "退费金额", [@foreach($ids as $id) "{{ $id }}", @endforeach], [@foreach($refund_amounts as $refund_amount) "{{ $refund_amount }}", @endforeach]);
</script>
@endsection
