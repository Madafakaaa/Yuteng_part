          ...
          <div class="card card-stats">
            <div class="card-body">
              <div class="row">
                ...
                <div class="col">
                  <h5 class="card-title text-muted mb-0">共计收入</h5>
                  <span class="h2 font-weight-bold mb-0 text-blue">￥ {{ number_format($total_income, 2) }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="card card-stats">
            <div class="card-body">
              <div class="row">
                ...
                <div class="col">
                  <h5 class="card-title text-muted mb-0">签约数量</h5>
                  <span class="h2 font-weight-bold mb-0 text-blue">{{ number_format($total_contract_num) }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="card card-stats">
            <div class="card-body">
              <div class="row">
                ...
                <div class="col">
                  <h5 class="card-title text-muted mb-0">售出课时</h5>
              ...
              <table class="table align-items-center table-hover text-left">
                <thead class="thead-light">
                  <tr>
                    ...
                    <th style="width:28%;">合计收入</th>
                    <th style="width:28%;">签单数量</th>
                    <th style="width:28%;">签约课时</th>
                  </tr>
                </thead>
                <tbody>
                  @for($i=0;$i<count($ids);$i++)
                    <tr>
                      <td>{{ $ids[$i] }}</td>
                      @if($incomes[$i]!=0)
                        <td>￥ {{ number_format($incomes[$i], 2) }}</td>
                      @else
                        <td></td>
                      @endif
                      @if($contract_nums[$i]!=0)
                        <td>{{ number_format($contract_nums[$i]) }}</td>
                      @else
                        <td></td>
                      @endif
                      @if($hours[$i]!=0)
...
<script>
  ...
  barChart("barChart1", "签约收入", [@foreach($ids as $id) "{{ $id }}", @endforeach], [@foreach($incomes as $income) "{{ $income }}", @endforeach]);
</script>
@endsection
