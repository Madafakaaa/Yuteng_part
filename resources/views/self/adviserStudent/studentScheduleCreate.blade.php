...
<div class="container-fluid mt-4">
  <div class="row justify-content-center">
    <div class="col-lg-10 col-md-10 col-sm-12 card-wrapper ct-example">
      <div class="card">
        <form action="/self/adviser/student/schedule/create2" method="post" id="form1" name="form1">
          @csrf
          <div class="card-header">
            <h3 class="mb-0">学生课程安排</h3>
          </div>
          <!-- Card body -->
          <div class="card-body">
            <div class="row">
              ...
              <div class="col-2 pl-2 pr-2 mb-2">
                ...
                <input type="hidden" name="input_student" value="{{ $student->student_id }}" required>
              </div>
              <div class="col-2 text-center">
                <label class="form-control-label">校区</label>
              </div>
              <div class="col-2 pl-2 pr-2 mb-2">
                <label>{{ $student->department_name }}</label>
              </div>
              <div class="col-2 text-center">
                <label class="form-control-label">年级</label>
              </div>
              <div class="col-2 pl-2 pr-2 mb-2">
                <label>{{ $student->grade_name }}</label>
              </div>
            </div>
            <hr>
            @if(count($classes)>0)
                ...
                <div class="form-group">
                  <select class="form-control form-control-sm" name="input_teacher" data-toggle="select" required>
                    ...
                    @foreach ($teachers as $teacher)
                      <option value="{{ $teacher->user_id }}">{{ $teacher->user_name }} ({{ $teacher->position_name }}@if($teacher->user_department!=$student->student_department) {{ $teacher->department_name }}@endif)</option>
                    @endforeach
                  </select>
                </div>
              </div>
