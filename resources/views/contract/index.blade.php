          ...
          <table class="table align-items-center table-hover text-left table-bordered">
            <thead class="thead-light">
              <tr>
                ...
                <th style='width:80px;' class="text-right">购买课时</th>
                <th style='width:80px;' class="text-right">赠送课时</th>
                <th style='width:120px;' class="text-right">原金额</th>
                <th style='width:120px;' class="text-right">优惠金额</th>
                <th style='width:80px;' class="text-right">合计课时</th>
            ...
            <tbody>
              ...
              @endif
              @foreach ($rows as $row)
              <tr title="创建时间：{{ $row->contract_createtime }}。">
                <td>{{ $startIndex+$loop->iteration }}</td>
                <td>{{ $row->department_name }}</td>
                <td>{{ $row->student_name }}</td>
                <td>{{ $row->grade_name }}</td>
                <td class="text-right">{{ $row->contract_course_num }} 种课程</td>
                <td class="text-right">{{ $row->contract_original_hour }} 课时</td>
                <td class="text-right">{{ $row->contract_free_hour }} 课时</td>
                <td class="text-right">{{ number_format($row->contract_original_price, 1) }} 元</td>
                <td class="text-right"><span style="color:red;">- {{ number_format($row->contract_discount_price, 1) }} 元</span></td>
                <td class="text-right"><strong>{{ $row->contract_total_hour }} 课时</strong></td>
                <td class="text-right"><strong>{{ number_format($row->contract_total_price, 1) }} 元</strong></td>
