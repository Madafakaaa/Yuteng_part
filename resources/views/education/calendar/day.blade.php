...
@section('nav')
<h2 class="text-white d-inline-block mb-0">课程表 @if (Session::get('user_access_self')==1) （个人） @endif</h2>
          ...
          <form action="" method="get" onsubmit="submitButtonDisable('submitButton1')">
            <div class="row">
              <div class="col-6">
                ...
                <a href="/education/calendar/week?@foreach($filters as $key => $value){{$key}}={{$value}}&@endforeach"><button type="button" class="btn btn-outline-primary btn-sm">返回周课程表</button></a>
              </div>
              <div class="col-3">
              </div>
        ...
        @foreach($rows as $row)
          {
            ...
            title: "{{$row['title']}}",
            body: "<a href='/education/schedule/attend?id={{$row['schedule_id']}}'><button type='button' class='btn btn-primary btn-sm' @if($row['attended']==1) disabled @endif>点名</button></a>",
            category: "time",
