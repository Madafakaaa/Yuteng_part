...
@section('nav')
<h2 class="text-white d-inline-block mb-0">签约管理 @if (Session::get('user_access_self')==1) （个人） @endif</h2>
...
<div class="container-fluid mt-3">
  <div class="row mb-3">
    <div class="col-auto">
      <a href="?@foreach($filters as $key => $value) @if($key!='filter_user') {{$key}}={{$value}}& @endif @endforeach&filter_user={{Session::get('user_id')}}" class="btn btn-sm btn-neutral btn-round btn-icon" data-toggle="tooltip" data-original-title="添加客户">
        <span class="btn-inner--icon"><i class="fas fa-user-circle"></i></span>
        <span class="btn-inner--text">我的签约</span>
      </a>
      <a href="?">
