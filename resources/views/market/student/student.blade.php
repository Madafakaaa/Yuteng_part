...
@section('nav')
<h2 class="text-white d-inline-block mb-0">学生管理 @if (Session::get('user_access_self')==1) （个人） @endif</h2>
          ...
          <table class="table align-items-center table-hover table-bordered text-left">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <a href="/market/student/contract/create?id={{encode($student['student_id'], 'student_id')}}"><button type="button" class="btn btn-warning btn-sm">签约合同</button></a>
                  <button type="button" class="btn btn-outline-danger btn-sm delete-button" id='delete_button_{{$loop->iteration}}' onclick="deleteConfirm('delete_button_{{$loop->iteration}}', '/market/student/delete?id={{encode($student['student_id'], 'student_id')}}', '确认删除学生？')">删除</button>
                </td>
              </tr>
              @endforeach
            </tbody>
