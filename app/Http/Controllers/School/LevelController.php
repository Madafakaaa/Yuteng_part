...
class LevelController extends Controller
{
    ...
    public function index(Request $request){
        ...
        // 获取数据库信息
        // 获取总数据数
        $totalRecord = DB::table('level')->where('level_status', 1);
        // 添加筛选条件
        // 等级名称
        if ($request->has('filter1')) {
            if($request->input('filter1')!=''){
                $totalRecord = $totalRecord->where('level_name', 'like', '%'.$request->input('filter1').'%');
            }
        }
        ...
        $rows = $rows->orderBy('level_createtime', 'asc')
                     ...
                     ->limit($rowPerPage)
                     ->get();
        return view('school/level/index', ['rows' => $rows, 'currentPage' => $currentPage, 'totalPage' => $totalPage, 'startIndex' => ($currentPage-1)*$rowPerPage]);
    }
