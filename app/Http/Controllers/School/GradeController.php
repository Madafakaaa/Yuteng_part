...
class GradeController extends Controller
{
    ...
    public function index(Request $request){
        ...
        // 获取数据库信息
        // 获取总数据数
        $totalRecord = DB::table('grade')->where('grade_status', 1);
        // 添加筛选条件
        // 年级名称
        if ($request->has('filter1')) {
            if($request->input('filter1')!=''){
                $totalRecord = $totalRecord->where('grade_name', 'like', '%'.$request->input('filter1').'%');
            }
        }
        ...
        $rows = $rows->orderBy('grade_createtime', 'asc')
                     ...
                     ->limit($rowPerPage)
                     ->get();
        return view('school/grade/index', ['rows' => $rows, 'currentPage' => $currentPage, 'totalPage' => $totalPage, 'startIndex' => ($currentPage-1)*$rowPerPage]);
    }
