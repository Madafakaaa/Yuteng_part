...
class MarketController extends Controller
{
    ...
    public function consultantEdit(Request $request){
        ...
        $users = DB::table('user')
                  ...
                  ->where('user_status', 1)
                  ->get();
        return view('market/consultantEdit', ['student' => $student, 'users' => $users]);
    }
    ...
    /**
     * 删除客户
     * URL: DELETE /market/customer/all/{student_id}
     * @param  int  $position_id
     */
    public function customerAllDelete($student_id){
        // 检查登录状态
        if(!Session::has('login')){
            return loginExpired(); // 未登录，返回登陆视图
        }
    ...
    /**
     * 删除我的客户
     * URL: DELETE /market/customer/my/{student_id}
     * @param  int  $position_id
     */
    public function customerMyDelete($student_id){
        // 检查登录状态
        if(!Session::has('login')){
            return loginExpired(); // 未登录，返回登陆视图
        }
    ...
    /**
     * 学生删除(转为离校)
     * URL: DELETE /market/student/all/{student_id}
    ...
    public function studentDelete($student_id){
        ...
        // 捕获异常
            ...
            return redirect("/market/student/all")
                     ->with(['notify' => true,
                             'type' => 'danger',
                             'title' => '学生删除失败',
    ...
    /**
     * 修改学生负责人提交
     * URL: POST /market/student/follower/store
     */
    ...
    /**
     * 离校学生恢复
     * URL: GET /market/student/deleted/restore/{student_id}
     * @param  int  $student_id
     */
    ...
    /**
     * 离校学生删除
     * URL: DELETE /market/student/deleted/{student_id}
     * @param  int  $student_id
     */
