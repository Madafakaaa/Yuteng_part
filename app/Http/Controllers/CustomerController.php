...
class CustomerController extends Controller
{
    ...
    public function create(){
        ...
        return view('customer/create', ['departments' => $departments,
                                               'sources' => $sources,
                                               'courses' => $courses,
                                               'users' => $users,
                                               'grades' => $grades]);
    ...
    public function show($customer_id){
        ...
        $customer = DB::table('customer')
                      ...
                      ->get();
        $follow_records = DB::table('follow_record')
                                     ->leftJoin('user', 'follow_record.follow_record_follower', '=', 'user.user_id')
                                     ->where('follow_record_customer', $customer_id)
                                     ->orderBy('follow_record_createtime', 'desc')
                                     ->get();
        if($customer->count()!==1){
            // 未获取到数据
            return redirect()->action('CustomerController@index')
    ...
    public function record(Request $request, $customer_id){
        ...
        // 获取表单输入
        $follow_record_follower = $request->input('input1');
        $follow_record_conversed = $request->input('input2');
        $follow_record_remark = $request->input('input3');
        $follow_record_customer = $customer_id;
        // 获取当前用户ID
        $follow_record_createuser = Session::get('user_id');
        // 获取数据信息
        $customer_name = DB::table('customer')->where('customer_id', $customer_id)->value('customer_name');
        // 更新数据
        try{
            DB::table('customer')->where('customer_id', $customer_id)->update(['customer_conversed' => $follow_record_conversed]);
            DB::table('customer')->where('customer_id', $customer_id)->increment('customer_follow_time');
            DB::table('follow_record')->insert(
                ['follow_record_customer' => $follow_record_customer,
                 'follow_record_follower' => $follow_record_follower,
                 'follow_record_conversed' => $follow_record_conversed,
                 'follow_record_remark' => $follow_record_remark,
                 'follow_record_createuser' => $follow_record_createuser]
            );
        }
        // 捕获异常
        catch(Exception $e){
