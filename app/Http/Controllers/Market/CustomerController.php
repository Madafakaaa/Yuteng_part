...
class CustomerController extends Controller
{
    ...
    public function customerCreate(){
        ...
        // 检测用户权限
        if(!in_array("/market/customer/create", Session::get('user_accesses'))){
           return back()->with(['notify' => true,'type' => 'danger','title' => '您的账户没有访问权限']);
        }
        // 获取用户校区权限
        $department_access = Session::get('department_access');
    ...
    public function customerDelete(Request $request){
        ...
        // 检测用户权限
        if(!in_array("/market/customer/delete", Session::get('user_accesses'))){
           return back()->with(['notify' => true,'type' => 'danger','title' => '您的账户没有访问权限']);
        }
        // 获取student_id
    ...
    public function customerContractCreate(Request $request){
        ...
        // 检测用户权限
        if(!in_array("/market/customer/contract/create", Session::get('user_accesses'))){
           return back()->with(['notify' => true,'type' => 'danger','title' => '您的账户没有访问权限']);
        }
        // 获取学生id
        ...
        return view('market/customer/customerContractCreate', ['student' => $student,
                                                                   'hours' => $hours,
    ...
    public function customerContractStore(Request $request){
        ...
        // 捕获异常
            ...
            DB::rollBack();
            // 返回购课界面
            return redirect("/market/customer/contract/create?id=".encode($request_student_id, 'student_id'))
                   ->with(['notify' => true,
                         'type' => 'danger',
                         'title' => '购课添加失败',
        ...
        return redirect("/market/customer/contract/success?student_id=".encode($contract_student, 'student_id')."&contract_id=".encode($contract_id, 'contract_id'))
               ->with(['notify' => true,
                       'type' => 'success',
                       'title' => '合同添加成功',
                       'message' => '合同添加成功']);
    ...
    public function customerContractSuccess(Request $request){
        return view('market/customer/customerContractCreateSuccess', ['student_id' => $request->input('student_id'), 'contract_id' => $request->input('contract_id')]);
    }
